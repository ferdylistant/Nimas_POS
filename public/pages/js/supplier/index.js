$(document).ready(function () {
    ...
    let tbSupplier = $('#tb_Supplier').DataTable({
        "responsive": true,
        "autoWidth": true,
        scrollX: true,
        scrollY: 300,
        ...
        columns: [
            ...
            { data: 'shopname', name: 'shopname', title: 'Toko/Instansi', className: 'text-center text-secondary text-sm' },
    ...
    async function ajaxClickLoadMoreHistory(id) {
        await $('#loadMore').click(function (e) {
            ...
            $.ajax({
                type: "GET",
                url: baseUrl + "/people/supplier/history/ajax-modal",
                data: {
                    id: id,
                    page: page
                },
    ...
    async function ajaxModalSupplier(el, type, id='', name='') {
        await $.ajax({
            type: "GET",
            url: baseUrl + "/people/supplier/" + type + "/ajax-modal",
            data: {
                id: id,
                name: name
            },
            success: function (result) {
                el.find('#titleModalSupplier').html(result.title);
                if (type != 'history') {
                    el.find(':submit').data('el', '#' + result.idForm);
                    el.find(':submit').attr('form', result.idForm);
                }
                el.find('#mainContent').html(result.html);
                let valid = jqueryValidation_("#fm_" + type + "Supplier", {
                    name: {
                        required: true,
                    },
                    email: {
    ...
    async function ajaxAddSupplier(el) {
        await $.ajax({
            type: "POST",
            url: baseUrl + "/people/supplier/store",
            ...
            success: function (result) {
                ...
                if (result.status == "success") {
                    $("#fm_addSupplier").trigger("reset");
                    $('#mdSupplier').modal('hide');
                    tbSupplier.ajax.reload();
                }
            },
            error: function (err) {
                console.log(err);
    ...
    async function ajaxEditSupplier(el) {
        await $.ajax({
            type: "POST",
            url: baseUrl + "/people/supplier/update",
            data: new FormData($(el).get(0)),
            processData: false,
            contentType: false,
            cache: false,
    ...
    async function ajaxDeleteProduct(id) {
        await $.ajax({
            type: "DELETE",
            url: baseUrl + "/people/supplier/delete/"+id,
            success: function (result) {
                notifToast(result.status, result.message);
                if (result.status == "success") {
                    tbSupplier.ajax.reload();
    ...
    $('#tb_Supplier').on('click', '.btnDeleteSupplier', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
    ...
    if (urlParams.has('modal')) {
        ...
        ajaxModalSupplier($('#mdSupplier'), urlParams.get('modal'), urlParams.get('id'), urlParams.get('name'));
    }
    $('#mdSupplier').on({
        ...
        'hide.bs.modal': function () {
            window.history.pushState({}, '', '/people/supplier');
        },
        'submit': function (e) {
            e.preventDefault();
            ...
            if (el.valid()) {
                switch (ele) {
                    case '#fm_addSupplier':
                        ...
                        var text = 'add supplier (' + val + ')?';
                        break;
                    default:
                        var title = 'Edit Supplier';
                        var text = 'edit supplier (' + val + ')?';
                        break;
                }
                $.confirm({
                    theme: 'modern',
